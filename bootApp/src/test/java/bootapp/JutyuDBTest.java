...
/**
 * 受注エンティティ/リポジトリ、明細エンティティ/リポジトリの メソッドをテストする.
 */
...
public class JutyuDBTest {
    ...
    public void findJutyu() {
        ...
        jRepo.jutyuBetuJutyugaku().stream().map(Arrays::toString).forEach(log::info);

        log.info(">>> 商品別、受注額一覧: syohinBetuJutyugaku()");
        jRepo.syohinBetuJutyugaku().stream().map(Arrays::toString).forEach(log::info);
