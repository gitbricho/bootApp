...
class TestUtil {
    ...
    Contact getContactLikeSimei(String srchStr) {
        List<Contact> contacts = contactRepo.findAll(data.getUser("tosi"), "家族親戚",
                srchStr);
    ...
    void logSetup() {
        log.debug("$$$$$$ テストの初期処理： ユーザー({}件), 連絡先({}件)を作成", data.getUserKensu(),
    ...
    void logAllContact(List<Contact> contacts, String rem) {
        ...
        contacts.forEach(c -> {
            log.info(c.toString());
            c.getAddresses().forEach(e -> {
                log.info(">>{}", e.toString());
            });
            c.getMails().forEach(e -> {
                log.info(">>{}", e.toString());
            });
            c.getPhones().forEach(e -> {
                log.info(">>{}", e.toString());
            });
            log.info("---------------------------");
        });
