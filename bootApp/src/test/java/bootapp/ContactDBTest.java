...
public class ContactDBTest {
    ...
    @Rule
    public TestWatchMan logRule = new TestWatchMan();
    ...
    public void saveContact() {
        ...
        Contact c = new Contact(null, user, "新規", "追加", "しんき", "ついか", 
        		"女", "1980/01/01", "家族親戚");
        c.getAddresses().add(
        		new Address(null, "1234444", "東京都", "千代田区", "", "", "", "自宅"));
        c.getMails().add(new Mail(null, "t.sinki@xmail.com", "自宅"));
        c.getMails().add(new Mail(null, "t.sinki@xxx.xx.com", "携帯"));
        c.getPhones().add(new Phone(null, "09011112222", "携帯"));
    ...
    public void updateContact02() {
        ...
        // 住所とメールを追加
        c.getAddresses().add(
        		new Address(null, "3335555", "東京都", "大田区", "", "", "", "自宅"));
        c.getMails().add(new Mail(null, "t.sinki@ssmail.com", "自宅"));
        contactRepo.save(c);
        // 更新後のチェック
    ...
    public void validMail() {
        ...
        Mail m = new Mail(null, "t.tamcho@gmail.com", "自宅");
    ...
    public void testInvalidMail_MailType() {
        Mail m = new Mail(null, "tt@gmail.com", "");	//メール種別が空
        Set<ConstraintViolation<Mail>> violations = validator.validate(m);
        assertThat(violations.size()).isEqualTo(1);
        ConstraintViolation<Mail> cv = violations.iterator().next();
    ...
    public void testInvalidPhone_TelNo() {
        ...
        Phone p = new Phone(null, "09011xxx", "自宅");
    ...
    public void testInvalidPhone_PhoneType() {
        Phone p = new Phone(null, "09011112222", "");	//連絡先種別が空
        Set<ConstraintViolation<Phone>> violations = validator.validate(p);
        assertThat(violations.size()).isEqualTo(1);
        ConstraintViolation<Phone> cv = violations.iterator().next();
